  ...
  ig.Hud = ig.Class.extend({
    ...
    draw: function() {
      ...
      if(this.respawnTimer) {
        ...
        this.font.draw( ig.game.waveDirector.remainingEnemies + ' Enemies', 5, 15, ig.Font.ALIGN.LEFT );
      ...
      if(ig.game.player) {
        var screenX = ig.system.width;
        var screenY = 5;
        for (var i = 0; i < ig.game.player.weapons.length; i++) {
          var weapon = ig.game.player.weapons[i];
          if(weapon instanceof WeaponPistol) continue;
          var txt = weapon.name + ' ammo: ' + weapon.ammo;

          this.font.draw(txt, screenX - 5, screenY, ig.Font.ALIGN.RIGHT);
          screenY += 10;
        }
      }
    },
