...
.defines(function() {
  ig.WaveDirector = ig.Class.extend({
    ...
    init: function( x, y, settings ) {
      ...
      this.ammoTimer = new ig.Timer(ig._.rndRange(10, 30));
    },
    ...
    update: function() {
      ...
      if(this.ammoTimer && this.ammoTimer.delta() >= 0) {
        this.ammoTimer = null;
        var spawner = ig.game.getEntitiesByType(EntityAmmoSpawnLocation).random();

        var self = this;
        spawner.spawnAmmo(function() {
          self.ammoTimer = new ig.Timer(ig._.rndRange(10, 30));
        });
      }
    },
