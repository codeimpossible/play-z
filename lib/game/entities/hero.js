...
.defines(function() {
  EntityHero = ig.Entity.extend({
    ...
    update: function() {
      ...
      if(this.isDodging) {
        ...
        if(this.dodgeTimer.delta() > GameConfig.playerDodgeTime) {
          this.isDodging = false; // might need to add a cooldown here to prevent spamming
        }
      } else { // the player can't do anything else while dodging
        if(nextWeapon) {
          this.currentWeapon++;
    ...
    shootRifle: function() {
      ...
      this.shootSound.play();
      ig.game.spawnEntity( ProjectileBulletSmall, pos.x + (16 * flipMod), pos.y, {
        flip: this.currentAnim.flip.x,
        vel: {
          x: this.vel.x + 180 * flipMod
        }
      });
      this.shootSound.play();
      ig.game.spawnEntity( ProjectileBulletSmall, pos.x + (8 * flipMod), pos.y, {
        flip: this.currentAnim.flip.x,
        vel: {
          x: this.vel.x + 180 * flipMod
        }
