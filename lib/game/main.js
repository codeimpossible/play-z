...
.requires(
  ...
  'game.levels.level1',
  'game.directors.logger',
  'game.directors.perf-mon',
  'game.levels.test'
)
.defines(function() {
  MyGame = ig.Game.extend({
    gravity: 9.8,
		...
		screenOffset: {
			x: 30,
			y: 1 // should be at least 1 to help with teh maths below
		},
    ...
    init: function() {
      ...
      // if running in the wild, collect perf stats randomly
      if(window.ga) {
        ig.PerfMon.setup();
      }
      ...
      if(this.player.climbingLadder() && this.player.ladderDir === 'down') {
        bufferY = this.screenOffset.y * -30;
      }

      if(this.player.peekDown) {
        bufferY = this.screenOffset.y * -30;
      }
  ...
  if(window.location.href.indexOf('localhost') === -1) {
    // include our logging director
    var tag = document.createElement('script');
    tag.src = 'lib/game/directors/logger.js';
